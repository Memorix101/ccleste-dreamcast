...
#endif
    SDL_Log(">> Setup Audio\n");
    SDL_CHECK(SDL_Init(SDL_INIT_AUDIO) == 0); // DC
#if SDL_MAJOR_VERSION >= 2 && ! defined (__NGAGE__) && ! defined (NGAGE_DEBUG)
    SDL_Log(">> Setup SDL_INIT_GAMECONTROLLER\n");
...
#if CELESTE_P8_ENABLE_AUDIO
    SDL_Log(">> Setup Mix_OpenAudio\n");
    /*if (Mix_Init(mixflag) != mixflag) // DC
    {
        ErrLog("Mix_Init: %s\n", Mix_GetError());
    }*/
...
skip_load:
    ...
    SDL_Log("Celeste_P8_init...");
    Celeste_P8_init();

    /*SDL_Log("ready");
    {
        SDL_Log("CCLESTE_START_FULLSCREEN...");
...
static void ReadGamepadInput(Uint16* out_buttons)
{
    ...
    if (!controller)
    {
        ...
        //SDL_Log(">> Detecting connected controllers ... ");
        for (i = 0; i < count; i++)
        {
...
for (i = 0; i < sizeof(controller_mappings) / sizeof(*controller_mappings); i++) 
{
    ...
    pressed = SDL_JoystickGetButton(controller, mapping.sdl_btn);

    // Log the button state
    /*SDL_Log("Button mapped to pico8_btn %d (SDL button %d): %s", 
            mapping.pico8_btn, 
            mapping.sdl_btn, 
...
// Joystick -> D-Pad
x_axis = SDL_JoystickGetAxis(controller, 0);
y_axis = SDL_JoystickGetAxis(controller, 1);
...
if (y_axis > stick_deadzone) { // down
    ...
    *out_buttons |= (1 << 3);
    ...
    if (x_axis < -stick_deadzone) //left
    {
        SDL_Log(">> controller left!");
        *out_buttons |= (1 << 0);
    }
    if (x_axis >  stick_deadzone) //right
    {
        SDL_Log(">> controller right!");
        *out_buttons |= (1 << 1);
    }
    if (y_axis < -stick_deadzone) //up
    {
